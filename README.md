...
**Live site:** https://sams.news &nbsp;·&nbsp; [☕ Donate](https://donate.stripe.com/8x2aEWacsatL7bq2IU8ww00)

## What it does

Each run (every 2 hours):

1. **Fetches** RSS + runs per-topic Google News searches to catch stories RSS missed
...
| Google News RSS | Per-topic discovery + per-story search (free, no key); redirect links resolved to publisher URLs |
...
1. Push to GitHub
2. Add repo secrets: `ANTHROPIC_API_KEY`, `VOYAGE_API_KEY` (plus `OPENAI_API_KEY` for the podcast)

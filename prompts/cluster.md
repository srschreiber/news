...
3. **Score importance 1–10** (decimals OK, e.g. 7.5) relative to the primary
   feed of each event (see **Feed scoring contexts** below), weighing breadth of
   impact, consequence, and novelty within that audience. Do not compare science
   stories against tech stories — judge each event against what matters to its
   own readers. Use the full range: reserve 9–10 for rare landmark events, use
   4–6 for solid but routine stories.
